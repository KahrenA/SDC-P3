...
int main()
{
  ...
  if (!read_map_data("../data/map_data.txt", map)) {
	...
	//std::cout << "Read Map data successful! \n";
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") 
	  {
        ...
        if (event == "telemetry") 
		{
          ...
          if (!pf.initialized()) 
		  {
				//std::cout << "About to read FIRST sense_x, sense_y and sense_theta\n\n";
		  ...
		  {
			//std::cout << "About to read SUBSEQUENT sense_x, sense_y and sense_theta\n\n";
		  ...
		  // sense_observations in JSON format [{obs_x,obs_y},{obs_x,obs_y},...{obs_x,obs_y}]
        	...
        	for(int i = 0; i < x_sense.size(); i++)
        	{
        		...
        		obs.x = x_sense[i];
				obs.y = y_sense[i];
				//std::cout << "OBS:i = " << i << "\t" << "x_sense[i] = " << x_sense[i] 
				//     			<< "\t" << "y_sense[i] = " << y_sense[i] << "\n";
